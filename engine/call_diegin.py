...
from evo.main import (get_rules_for_task, arbitrate, full_review, record_behavior,
                      health_check, run_maintenance, mempalace_archive, mempalace_search,
...
def pre_check(context: dict) -> dict:
    ...
    decision = arbitrate(rules["interceptions"], rules["patterns"])
    return {
        "matched_interceptions": len(rules["interceptions"]),
        "matched_patterns": len(rules["patterns"]),
        "decision": decision["decision"],
        "reason": decision["reason"],
        "winning_rule_id": decision["winning_rule"].get("id") if decision["winning_rule"] else None,
    }
...
if __name__ == "__main__":
    ...
    if mode == "check" or mode == "stdin":
        ...
        ok = mempalace_archive(content, source)
