...
def get_seed_interceptions() -> List[InterceptionRule]:
    ...
    return [
        InterceptionRule(
            id="rule_risk_gate_001",
            trigger_condition="leverage_ratio > 1.0",
            action="block_execution; force_risk_checklist",
            severity="high",
            tags=["risk_control", "irreversible"],
            logic_score=5.0,
            outcome_score=5.0,
            confidence=5.0,
            source="seed",
            source_review="system_init",
            created_at=now,
            valid_until="",
            lifecycle_status="active",
            boundary_conditions=["杠杆数据不可靠时降级为手动确认"],
            invalid_conditions=[]
        ),
        InterceptionRule(
            id="rule_risk_gate_002",
            trigger_condition="exposure_per_single_item > 0.2",
            action="block_execution; force_limit_checklist",
            severity="high",
            tags=["risk_control", "irreversible"],
            logic_score=5.0,
            outcome_score=5.0,
            ...
            valid_until="",
            lifecycle_status="active",
            boundary_conditions=["风险限制可动态调整，需用户确认"],
            invalid_conditions=[]
        ),
        InterceptionRule(
            id="rule_risk_gate_003",
            trigger_condition="task.type == 'order_submit' AND order.amount > daily_budget * 0.3",
            action="block_execution; force_budget_approval",
            severity="medium",
            tags=["risk_control"],
            logic_score=5.0,
            outcome_score=5.0,
